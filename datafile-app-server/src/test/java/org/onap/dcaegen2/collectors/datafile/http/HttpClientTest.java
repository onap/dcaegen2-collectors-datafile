...
public class HttpClientTest {
  ...
  @Test
  public void openConnection_successAuthSetup() {
  ...
  public void collectFile_AllOk() throws Exception {
    ...
    when(dfcHttpClientSpy.getServerResponse(any(), any())).thenReturn(fis);
    doReturn(true).when(dfcHttpClientSpy).isResponseOk(any());
    ...
    verify(dfcHttpClientSpy, times(1)).processDataFromServer(any(), any(), any(), any());
    verify(dfcHttpClientSpy, times(1)).getServerResponse(ArgumentMatchers.eq(REMOTE_FILE), any());
    verify(dfcHttpClientSpy, times(1)).isResponseOk(any());
    verify(dfcHttpClientSpy, times(1)).isDownloadFailed(any());
  }

  @Test
  public void collectFile_No200ResponseWriteToErrorMessage() {
    String ERROR_RESPONSE = "This is unexpected message";
    String REMOTE_FILE = "any";
    Flux<InputStream> fis = Flux.just(new ByteArrayInputStream(ERROR_RESPONSE.getBytes()));

    dfcHttpClientSpy.open();

    when(dfcHttpClientSpy.getServerResponse(any(), any())).thenReturn(fis);
    doReturn(false).when(dfcHttpClientSpy).isResponseOk(any());
