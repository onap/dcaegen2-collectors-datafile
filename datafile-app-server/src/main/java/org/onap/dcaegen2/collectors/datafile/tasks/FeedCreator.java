...
/**
 * Stateless component that creates the feed used to publish files in DataRouter.
 *
...
public class FeedCreator {
     ...
     */
    public Mono<FeedData> execute(long numRetries, Duration firstBackoff) {
        logger.trace("Executing");
        DmaapBusControllerReactiveHttpClient busControllerHttpClient = resolveClient();

        return Mono.just(datafileAppConfig.getDmaapBusControllerConfiguration().dmaapDrFeedName()) //
                .flatMap(feedName -> createFeed(feedName, busControllerHttpClient)) //
                .flatMap(this::handleHttpResponse) //
                .retryBackoff(numRetries, firstBackoff);
    }

    private Mono<HttpResponse> createFeed(String feedName,
            DmaapBusControllerReactiveHttpClient busControllerHttpClient) {
    ...
    private void prepareHead(HttpPost post, DmaapBusControllerReactiveHttpClient busControllerHttpClient) {
    ...
    DmaapBusControllerReactiveHttpClient resolveClient() {
        return new DmaapBusControllerReactiveHttpClient(resolveConfiguration());
    }
}
