...
public class HttpsClientConnectionManagerUtil {
    ...
    public static void setupOrUpdate(Path keyCertPath, String keyCertPasswordPath, String trustedCaPath,
    ...
    private static void setup(Path keyCertPath, String keyCertPasswordPath, String trustedCaPath,
          String trustedCaPasswordPath) throws DatafileTaskException {
        try {
            SSLContextBuilder sslBuilder = SSLContexts.custom();
            sslBuilder = supplyKeyInfo(keyCertPath, keyCertPasswordPath, sslBuilder);
            sslBuilder = supplyTrustInfo(trustedCaPasswordPath, trustedCaPath, sslBuilder);
    ...
    private static SSLContextBuilder supplyTrustInfo(String trustedCaPasswordPath, String trustedCaPath,
            SSLContextBuilder sslBuilder)
            throws NoSuchAlgorithmException, KeyStoreException, CertificateException, IOException {
        String trustPass = SecurityUtil.getPasswordFromFile(trustedCaPasswordPath, "Truststore");
        File trustStoreFile = new File(trustedCaPath);
        return sslBuilder.loadTrustMaterial(trustStoreFile, trustPass.toCharArray());
    }

    private static SSLContextBuilder supplyKeyInfo(Path keyCertPath, String keyCertPasswordPath,
            SSLContextBuilder sslBuilder)
            throws IOException, KeyStoreException, NoSuchAlgorithmException, CertificateException,
            UnrecoverableKeyException {
        String keyPass = SecurityUtil.getPasswordFromFile(keyCertPasswordPath, "Keystore");
        KeyStore keyFile = createKeyStore(keyCertPath, keyPass);
        return sslBuilder.loadKeyMaterial(keyFile, keyPass.toCharArray());
    }

    private static KeyStore createKeyStore(Path trustedCaPath, String trustedCaPassword)
            throws IOException, KeyStoreException, NoSuchAlgorithmException, CertificateException {
    ...
    protected static InputStream createInputStream(Path localFileName) throws IOException {
