...
public class DfcHttpClient implements FileCollectClient {
    ...
    private static final ConnectionProvider pool = ConnectionProvider.create("default",
            ScheduledTasks.NUMBER_OF_WORKER_THREADS);
    ...
    @Override public void collectFile(String remoteFile, Path localFile) throws DatafileTaskException {
        ...
        final AtomicReference<Throwable> errorMessages = new AtomicReference<>();
        ...
        if (isDownloadFailed(errorMessages)) {
            throw new DatafileTaskException("Error occured during datafile download: " + errorMessages.get().getCause());
        }

        logger.trace("HTTP collectFile OK");
    }

    protected boolean isDownloadFailed(AtomicReference<Throwable> errorMessage) {
        return (errorMessage.get() != null);
    }

    @NotNull protected Consumer<Throwable> processFailedConnectionWithServer(CountDownLatch latch, AtomicReference<Throwable> errorMessages) {
        return (Throwable response) -> {
            errorMessages.set(new Throwable("Error in connection has occurred during file download", response));
            latch.countDown();
        };
    }

    @NotNull protected Consumer<InputStream> processDataFromServer(Path localFile, CountDownLatch latch,
            AtomicReference<Throwable> errorMessages) {
        return (InputStream response) -> {
            ...
            try {
                ...
                errorMessages.set(new Throwable("Error fetching file with", e));
            } finally {
                latch.countDown();
            }
        };
