...
public class AppConfig {
    ...
    private Mono<AppConfig> fetchConfiguration(EnvProperties env) {
        Mono<JsonObject> serviceCfg = createCbsClient(env)
            .flatMap(cbsClient -> cbsClient.get(CbsRequests.getConfiguration(RequestDiagnosticContext.create())))
            .onErrorResume(AppConfig::onErrorResume);

        Mono<JsonObject> dmaapCfg = createCbsClient(env)
            .flatMap(cbsClient -> cbsClient.get(CbsRequests.getByKey(RequestDiagnosticContext.create(), "dmaap")))
